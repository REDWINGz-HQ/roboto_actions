...
echo "Pushing tag_console_ulog:latest to Roboto's private registry"
image_push_args=(
...
image_push_args+=(tag_console_ulog:latest)
image_push_ret_code=0
image_uri=$($roboto_exe "${image_push_args[@]}")
image_push_ret_code=$?

if [ $image_push_ret_code -ne 0 ]; then
    echo "Failed to push tag_console_ulog:latest to Roboto's private registry"
    exit 1
fi

echo "Creating tag_console_ulog action"
create_args=(
